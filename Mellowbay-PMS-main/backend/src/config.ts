...
// on: `vite dev` (3000), `vite preview` (4173), Vite's own default (5173), and
// the booking site (5174).
//
// Each also gets the port above it. Vite does not fail when its port is taken —
// it silently moves up one — so a developer with anything else on 3000 gets an
// app on 3001 that looks, from inside it, exactly like a dead server. Allowing
// the +1 costs nothing and saves that hour.
...
const SCHEMA = {
  // ─── Server ────────────────────────────────────────────────
  apiUrl: {
    env: 'API', kind: url(), fallback: 'http://localhost:8080',
    group: 'Tooling',
    doc: 'Base URL used by backend verification and demo scripts.',
    example: 'http://localhost:8080',
  },

  appUrl: {
    env: 'APP', kind: url(), fallback: 'http://localhost:4173',
    group: 'Tooling',
    doc: 'Front-end URL used by browser-based backend checks.',
    example: 'http://localhost:4173',
  },

  cdpUrl: {
    env: 'CDP', kind: url(), fallback: 'http://localhost:9222',
    group: 'Tooling',
    doc: 'Chrome DevTools Protocol URL used by browser-based checks.',
    example: 'http://localhost:9222',
  },

  bookingSiteUrl: {
    env: 'HELIO_BOOKING_SITE_URL', kind: url(), fallback: 'http://localhost:5173',
    group: 'Server',
    doc: 'Public address of the guest booking site. The check-in QR code points '
      + 'at /register on this host, so it must be an address the guest\'s own '
      + 'phone can open — a localhost address reaches the phone itself and the '
      + 'check-in screen says so rather than printing a code that cannot work.',
    example: 'https://book.example.com',
  },

  isoPortOffset: {
    env: 'ISO_PORT_OFFSET', kind: int({ min: 0, max: 1000 }), fallback: 0,
    group: 'Tooling',
    doc: 'Port offset used to isolate the concurrency verification server.',
    example: '0',
  },
  ...
  // ─── Channel manager ───────────────────────────────────────
  beds24Api: {
    env: 'BEDS24_API', kind: url(), fallback: 'https://beds24.com/api/v2',
  ...
  demoPassword: {
    env: 'DEMO_PASSWORD', kind: str({ min: 8 }),
  ...
  demoDormRooms: {
    env: 'DEMO_DORM_ROOMS', kind: int({ min: 1, max: 20 }), fallback: 3,
    group: 'Development scripts',
    doc: 'Number of dorm rooms created by the demo script.',
  },
  demoDormBedsPerRoom: {
    env: 'DEMO_DORM_BEDS_PER_ROOM', kind: int({ min: 1, max: 40 }), fallback: 8,
    group: 'Development scripts',
    doc: 'Number of beds created in each demo dorm room.',
  },
  demoHistoryDays: {
    env: 'DEMO_HISTORY_DAYS', kind: int({ min: 0, max: 365 }), fallback: 14,
    group: 'Development scripts',
    doc: 'Number of historical business days created by the demo script.',
  },
  smokeEmail: {
  ...
  smokePassword: {
    env: 'SMOKE_PASSWORD', kind: str({ min: 8 }),
    group: 'Development scripts', secret: true,
 ...
 */
const OWNED_PREFIXES = ['HELIO_', 'BEDS24_', 'DEMO_', 'SMOKE_', 'API', 'APP', 'CDP', 'ISO_'];

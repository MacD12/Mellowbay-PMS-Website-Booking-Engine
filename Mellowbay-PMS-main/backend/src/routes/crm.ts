...
import { listDocumentsForProfile } from '../services/documents.ts';
...
/**
 * The identity documents and signatures collected across this guest's stays.
 *
 * Metadata only — the images are fetched one at a time through
 * `/api/documents/:id`, which writes an audit entry, because "who looked at
 * this guest's passport, and when" is a question worth being able to answer.
 * `profiles.read` is not enough on its own for that; reading a document still
 * requires the permission the document route asks for.
 */
router.get('/api/profiles/:id/documents', (ctx: Ctx) => {
  const p = get<any>('SELECT id FROM profiles WHERE id = ? AND property_id = ?',
    ctx.params.id, pid(ctx));
  if (!p) notFound('Profile');
  return {
    documents: listDocumentsForProfile(pid(ctx), ctx.params.id),
    retentionDays: config.documentRetentionDays,
  };
}, { perm: 'profiles.read' });

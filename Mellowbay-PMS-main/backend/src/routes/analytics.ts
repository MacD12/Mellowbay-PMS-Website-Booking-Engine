...
router.get('/api/dashboard', (ctx: Ctx) => {
  ...
  const dirtyRooms = scalar<number>(
    ...
    pid(ctx));
  // "No room assigned" means a booking the property has taken and not yet put
  // in a room. A website booking still awaiting confirmation has no room
  // because it is not supposed to have one yet, so counting it here would send
  // the desk chasing work that does not exist.
  const unassigned = scalar<number>(
    `SELECT count(*) AS n FROM reservations r
      WHERE r.property_id = ? AND r.arrival = ? AND r.room_id IS NULL
        AND r.status IN ('Tentative','Confirmed','Guaranteed')
        AND ${NOT_PENDING_SQL}`, pid(ctx), today);

...
import { NOT_PENDING_SQL } from '../services/pendingbookings.ts';
...
import { assertModelFree, ensurePackagePlans, isBookingModel } from '../services/packages.ts';
  ...
  const unassigned = all<any>(
    `SELECT r.id, r.confirmation, r.guest_name, r.status, r.arrival, r.departure, r.vip,
      ...
      WHERE r.property_id = ? AND r.room_id IS NULL AND r.departure > ? AND r.arrival < ?
        ...
        -- A website booking awaiting confirmation is not an unassigned booking
        -- waiting for a room: it is not yet a booking the property has taken.
        -- Listing it here would invite the chart to drag it onto a bed, which
        -- would hold the bed without anyone having accepted the booking.
        AND ${NOT_PENDING_SQL}
      ORDER BY r.arrival`,
    pid(ctx), from, to,
  );
...
function shapePlan(p: any) {
  return {
    ...
    // Which booking-engine package this plan prices, if any. Null is a plain
    // room rate — every plan the front desk sells from.
    bookingModel: isBookingModel(p.booking_model) ? p.booking_model : null,
...
/**
 * Create the booking-engine package plans the property is missing.
 *
 * Idempotent, so the button behind it can be pressed twice without making a
 * second set of plans.
 */
router.post('/api/rate-plans/packages', (ctx: Ctx) => tx(() => {
  const result = ensurePackagePlans(pid(ctx));
  if (result.created.length) {
    audit(ctx.auth, {
      action: 'rateplan.packages', entity: 'RATE_PLAN', entityRef: result.created.join(','),
      after: result,
    }, ctx.ip);
  }
  return {
    ...
    plans: all<any>(
      `SELECT rp.*, parent.code AS parent_code FROM rate_plans rp
         LEFT JOIN rate_plans parent ON parent.id = rp.parent_id
        WHERE rp.property_id = ? AND rp.booking_model IS NOT NULL
...
router.post('/api/rate-plans', (ctx: Ctx) => tx(() => {
  const b = ctx.body;
  const planId = id('rp');
  const bookingModel = b.bookingModel ? b.bookingModel : null;
  assertModelFree(pid(ctx), bookingModel);
  run(
    `INSERT INTO rate_plans(id, property_id, code, name, description, parent_id, offset_type,
                            offset_value, refundable, flexible, kind, booking_model,
                            market_segment, min_los, max_los,
    ...
    // A plan that sells a booking-engine package is a package, whatever the
    // caller said its kind was.
    bookingModel
      ? 'package'
...
router.patch('/api/rate-plans/:id', (ctx: Ctx) => tx(() => {
  ...
  if (ctx.body.bookingModel !== undefined) {
    const model = ctx.body.bookingModel ? ctx.body.bookingModel : null;
    assertModelFree(pid(ctx), model, ctx.params.id);
    sets.push('booking_model = ?');
    args.push(model);
    // Taking a plan off its model leaves its kind alone — it may have been a
    // package before the booking engine existed. An explicit `kind` in the same
    // request wins, so this cannot assign the column twice.
    if (model && ctx.body.kind === undefined) { sets.push('kind = ?'); args.push('package'); }
  }
  ...
  for (const rt of roomTypes) {
    ...
    for (const rp of plans) {
      ...
      rows.push({
        ...
        // Set on a row that prices a booking-engine package rather than a room.
        bookingModel: isBookingModel(rp.booking_model) ? rp.booking_model : null,

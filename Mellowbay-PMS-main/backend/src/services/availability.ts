...
/**
 * The test every query below applies to a `reservations` row aliased `r`.
 *
 * A live status is necessary but not sufficient: a booking-engine booking that
 * nobody has confirmed yet holds nothing, so the website keeps offering the bed
 * until a person accepts the booking and picks one. See `pendingbookings.ts`.
 */
...
export function availabilityGrid(
  ...
  const pickRows = all<{ group_id: string; room_type_id: string; date: string; n: number }>(
    `SELECT r.group_id, n.room_type_id, n.date, count(*) AS n
       ...
       JOIN reservations r ON r.id = n.reservation_id
      WHERE n.property_id = ? AND n.date >= ? AND n.date < ?
        AND r.group_id IS NOT NULL AND ${LIVE_SQL}

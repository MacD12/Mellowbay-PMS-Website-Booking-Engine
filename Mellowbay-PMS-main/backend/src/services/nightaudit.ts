...
import { PENDING_SQL } from './pendingbookings.ts';
...
export function preflight(propertyId: string): {
  ...
  const pendingArrivals = all<any>(
    `SELECT r.id, r.confirmation, r.guest_name, ${PENDING_SQL} AS unconfirmed
       FROM reservations r
      WHERE r.property_id = ? AND r.arrival <= ?
        AND r.status IN ('Tentative','Confirmed','Guaranteed')`,
    propertyId, date,
  );
  if (pendingArrivals.length) {
    // Website bookings nobody confirmed are named separately, because they are
    // about to be closed out for a different reason and without a charge, and
    // the person deciding whether to run the audit should see that first.
    const unconfirmed = pendingArrivals.filter((r) => r.unconfirmed).length;
    issues.push({
      ...
      message: `${pendingArrivals.length} arrival(s) never checked in — they will be marked no-show`
        + (unconfirmed
          ? `, including ${unconfirmed} website booking(s) never confirmed (closed out with no charge)`
          : ''),
      items: pendingArrivals.map((r) => ({
        id: r.id,
        label: `${r.confirmation} · ${r.guest_name}${r.unconfirmed ? ' · awaiting confirmation' : ''}`,
      })),
    });
  }
...
export function runNightAudit(
  ...
  return tx(() => {
    ...
    let noShows = 0;
    const pendingArrivals = all<any>(
      `SELECT r.id, r.confirmation, r.guest_name, r.total_minor, r.nights,
              ${PENDING_SQL} AS unconfirmed
         FROM reservations r
        WHERE r.property_id = ? AND r.arrival <= ?
          AND r.status IN ('Tentative','Confirmed','Guaranteed')`,
      propertyId, date,
    ...
    for (const r of pendingArrivals) {
      // A no-show charge is for a guest who had a room held and did not come.
      // A website booking nobody confirmed never had one — the property, not
      // the guest, is why it did not happen — so it is closed out without a
      // charge whatever the policy says.
      const charge = r.unconfirmed || opts.noShowChargePolicy === 'none'
        ? 0

...
import { NOT_PENDING_SQL } from './pendingbookings.ts';
...
export function forecast(propertyId: string, date: string) {
  ...
  // Rooms to have ready tomorrow. A website booking still awaiting
  // confirmation has no room to prepare, so it is not housekeeping's workload

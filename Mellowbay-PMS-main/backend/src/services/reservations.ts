...
export function createReservation(propertyId: string, actor: Actor, input: CreateReservationInput) {
  return tx(() => {
    ...
    // A booking still awaiting confirmation holds no inventory, so nothing
    // about the property's availability has changed and the channels have
    // nothing to be told. Pushing here would shut dates on the OTAs for a
    // booking a person may yet decline — see `pendingbookings.ts`. The push and
    // the overbooking guard run when it is confirmed instead.
...
export function confirmBooking(
  ...
  return tx(() => {
    ...
    const roomType = get<any>('SELECT * FROM room_types WHERE id = ?', pendingMembers[0].room_type_id);
    const needsUnit = (a: ConfirmAssignment | undefined) =>
      !(roomType?.kind === 'dorm' ? a?.bedId : a?.roomId);
    if (!opts.auto && pendingMembers.some((m) => needsUnit(wanted.get(m.id)))) {
      throw new HttpError(400,
        roomType?.kind === 'dorm'
    ...
    for (const member of pendingMembers) {
      run(`UPDATE reservations SET status = 'Confirmed', updated_at = ? WHERE id = ?`,
        nowIso(), member.id);
    ...
    queueChannelPush(propertyId, lead.room_type_id, lead.arrival, lead.departure, 'reservation.confirm');
    guardAfter(propertyId, actor, lead.room_type_id, lead.arrival, lead.departure,

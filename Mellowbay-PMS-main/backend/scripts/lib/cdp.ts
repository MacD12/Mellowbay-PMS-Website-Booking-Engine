...
export const APP = config.appUrl;
export const API = config.apiUrl;
export const CDP_URL = config.cdpUrl;
...
export async function openApp(opts: { email?: string; password?: string; path?: string } = {}) {
  const email = opts.email ?? config.smokeEmail;
  const password = opts.password ?? config.smokePassword;

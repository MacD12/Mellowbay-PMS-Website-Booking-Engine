...
const DEMO_EMAIL = config.demoEmail;
...
async function main() {
  ...
  const packages = await runScript('packages-check.ts', standalone);
  out(`${packages.tail}\n`);
  results.push({ name: 'Booking-engine packages', ok: packages.ok, tail: packages.tail });

...
async function main() {
  ...
  if (!config.beds24RefreshToken?.trim()) {
...
async function createRoomTypes(propertyId: string, units: any[]) {
  ...
  const tokenRes = await fetch(`${config.beds24Api}/authentication/token`, {
    method: 'GET',
    headers: { accept: 'application/json', refreshToken: config.beds24RefreshToken ?? '' },
  });
  const tokenText = await tokenRes.text();
  let tokenBody: any;
  try { tokenBody = tokenText ? JSON.parse(tokenText) : null; } catch {
    fail(`Beds24 returned non-JSON while reading prices (${tokenRes.status}).`);
  }
  if (!tokenRes.ok || typeof tokenBody?.token !== 'string' || !tokenBody.token) {
    fail(`Beds24 refused the token while reading prices (${tokenRes.status} ${tokenRes.statusText}).`);
  }
  const token = tokenBody.token as string;

  const calRes = await fetch(
    `${config.beds24Api}/inventory/rooms/calendar`

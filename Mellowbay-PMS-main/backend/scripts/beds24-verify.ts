...
async function beds24Token(): Promise<string> {
  const rt = config.beds24RefreshToken?.trim();
  if (!rt) fail('BEDS24_REFRESH_TOKEN is not set.');
  const r = await fetch(`${BASE}/authentication/token`, {
    method: 'GET',
    headers: { accept: 'application/json', refreshToken: rt },
  });
  const text = await r.text();
  let body: any;
  try { body = text ? JSON.parse(text) : null; } catch {
    fail(`Beds24 returned non-JSON while requesting an access token (${r.status}).`);
  }
  if (!r.ok || typeof body?.token !== 'string' || !body.token) {
    fail(`Beds24 refused the token: ${r.status} ${r.statusText} ${JSON.stringify(body).slice(0, 160)}`);
  }
  return body.token;
}
